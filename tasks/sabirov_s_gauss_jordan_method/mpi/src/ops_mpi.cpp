...
bool SabirovSGaussJordanMethodMPI::PreProcessingImpl() {
  ...
  for (int proc = 0; proc < cached_world_size_; ++proc) {
    row_displs_[proc] = offset;
    size_t rows = base_rows + (std::cmp_less(proc, remainder) ? 1 : 0);
    row_counts_int_[proc] = static_cast<int>(rows);
    row_displs_int_[proc] = static_cast<int>(offset);
    offset += rows;
  }
...
std::pair<double, int> SabirovSGaussJordanMethodMPI::FindLocalPivot(size_t step, size_t cols) const {
...
void SabirovSGaussJordanMethodMPI::UpdateRowOrder(size_t pivot_row_id, size_t step) {
...
void SabirovSGaussJordanMethodMPI::NormalizePivotRow(size_t pivot_row_id, size_t step, size_t cols) {
  ...
  for (size_t j = 0; j < cols; ++j) {
    row_ptr[j] = pivot_row_buffer_[j];
  }
}

void SabirovSGaussJordanMethodMPI::BroadcastPivotRow(size_t pivot_row_id, int pivot_owner, size_t cols) {
...
void SabirovSGaussJordanMethodMPI::EliminateColumnMPI(size_t pivot_row_id, size_t step, size_t cols) {
  for (size_t pos = 0; pos < n_; ++pos) {
    size_t row_id = row_order_[pos];
    if (row_id == pivot_row_id || row_id < local_row_start_ || row_id >= local_row_end_) {
      continue;
    }
...
bool SabirovSGaussJordanMethodMPI::RunImpl() {
  ...
  for (size_t step = 0; step < n_; ++step) {
    auto [local_max, local_row] = FindLocalPivot(step, cols);
    ...
    auto pivot_row_id = static_cast<size_t>(global_pair.row);
    UpdateRowOrder(pivot_row_id, step);

    pivot_row_id = row_order_[step];
    const int pivot_owner = GetRowOwner(pivot_row_id);

    if (cached_rank_ == pivot_owner) {
      NormalizePivotRow(pivot_row_id, step, cols);
    }

    BroadcastPivotRow(pivot_row_id, pivot_owner, cols);
    EliminateColumnMPI(pivot_row_id, step, cols);
  }
...
size_t SabirovSGaussJordanMethodMPI::FindPivotRowSingle(size_t col, size_t cols) const {
...
void SabirovSGaussJordanMethodMPI::SwapRowsSingle(size_t row1, size_t row2, size_t cols) {
  for (size_t j = 0; j < cols; ++j) {
    std::swap(matrix_[(row1 * cols) + j], matrix_[(row2 * cols) + j]);
  }
}

void SabirovSGaussJordanMethodMPI::NormalizeRowSingle(size_t row, size_t col, size_t cols) {
...
void SabirovSGaussJordanMethodMPI::EliminateColumnSingle(size_t pivot_row, size_t col, size_t cols) {
...
bool SabirovSGaussJordanMethodMPI::RunSingleProcessGaussJordan() {
  ...
  for (size_t i = 0; i < n_; ++i) {
    size_t pivot_row = FindPivotRowSingle(i, cols);
    ...
    if (pivot_row != i) {
      SwapRowsSingle(i, pivot_row, cols);
    }

    NormalizeRowSingle(i, i, cols);
    EliminateColumnSingle(i, i, cols);
  }
...
int SabirovSGaussJordanMethodMPI::GetRowOwner(size_t global_row) const {
  int idx = 0;
  for (int rank = 0; rank < cached_world_size_; ++rank) {
    if (row_displs_[rank + 1] > global_row) {
      idx = rank;
      break;
    }
  }
