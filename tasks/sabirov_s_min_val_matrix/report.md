...
auto generate_value = [](int64_t i, int64_t j) -> InType {
  constexpr int64_t A = 1103515245;  // Множитель (из glibc)
  constexpr int64_t C = 12345;       // Инкремент
  constexpr int64_t M = 2147483648;  // Модуль (2^31)
  
  // Уникальный seed для каждой ячейки
  int64_t seed = (i * 100000007LL + j * 1000000009LL) ^ 42;
  
  // Применяем LCG формулу
...
bool SabirovSMinValMatrixMPI::RunImpl() {
  ...
  // Генерация данных через LCG и поиск минимумов
  // В реальной реализации используется generate_value(i, j) — см. раздел 2.4
  // Здесь показан упрощенный вариант для наглядности
  auto generate_value = [](int64_t i, int64_t j) -> InType {
...
auto generate_value = [](int64_t i, int64_t j) -> InType {
  ...
  constexpr int64_t A = 1103515245;  // Множитель
  constexpr int64_t C = 12345;       // Инкремент
  constexpr int64_t M = 2147483648;  // Модуль (2^31)
